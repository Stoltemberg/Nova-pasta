...
class HanbotMenu:
    def __init__(self):
        ...
        # Main Thread Custom Loop para permitir comandos do Win32/GLFW de forma Thread-Safe!
        import win32gui
        import win32con
        
        while dpg.is_dearpygui_running():
            if self.toggle_requested:
                hwnd = win32gui.FindWindow(None, "Hanbot Overlay")
                if hwnd:
                    if self.is_visible:
        ...
        while dpg.is_dearpygui_running():
            if self.bot and self.bot.running:
                ...
                if self.bot.engine:
                    ...
                    if target:
                        col = (255, 100, 100) if ttype.name == "CHAMPION" else (100, 255, 255)
                        dpg.configure_item("lbl_aim", default_value=f"[{ttype.name}] Mirando X:{target[0]} Y:{target[1]}", color=col)
                    else:
